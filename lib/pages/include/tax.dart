...
class CleanlyTax extends StatefulWidget {
  const CleanlyTax({Key ? key}) : super(key: key);

  @override
  _CleanlyTaxState createState() => _CleanlyTaxState();
}
...
class _CleanlyTaxState extends State<CleanlyTax>{
  // call Auth method
  ...
  String bodyMessage = '';
  List thead = ['Order No.', 'Subtotal', 'Discount', 'Tax', 'Total'];
  List tbody = [];
  List tfoot = [];
  ...
  Future<void> getApi(String period) async {
    ...
    try{
      http.Response response = await http.post(
          Uri.parse('${baseUrl}cleanly/v1/tax'),
          body: {
            'key': apiKey,
            'period': period,
          }
            ...
            VxBox(
                child: Row(
                  ...
                  children: [
                    ...
                    Expanded(flex:1,child: 'Subtotal'.text.center.size(12).bold.fontFamily('Bricolage').make(),),
                    Expanded(flex:1,child: 'Discount'.text.size(12).center.bold.fontFamily('Bricolage').make(),),
                    Expanded(flex:1,child: 'Tax'.text.size(12).center.bold.fontFamily('Bricolage').make(),),
