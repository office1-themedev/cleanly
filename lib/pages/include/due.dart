...
class CleanlyDue extends StatefulWidget {
  const CleanlyDue({Key ? key}) : super(key: key);

  @override
  _CleanlyDueState createState() => _CleanlyDueState();
}
...
class _CleanlyDueState extends State<CleanlyDue>{
  ...
  String titleReport = 'Due Reports';
  ...
  Future<void> getApi(String period) async {
    ...
    try{
      http.Response response = await http.post(
          Uri.parse('${baseUrl}cleanly/v1/due'),
          body: {
            'key': apiKey,
            'period': period,
          }
