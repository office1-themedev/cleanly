...
class CleanlyDeposit extends StatefulWidget {
  const CleanlyDeposit({Key ? key}) : super(key: key);

  @override
  _CleanlyDepositState createState() => _CleanlyDepositState();
}

class Person {
  Person(this.order, this.total, this.deposit, this.due);
...
class _CleanlyDepositState extends State<CleanlyDeposit>{
  ...
  String titleReport = 'Deposit Reports';
  String periodReport = 'Reporting Period:';
  String bodyMessage = '';
  List thead = ['Order No.', 'Total', 'Deposit', 'Due'];
  List tbody = [];
  List tfoot = [];
  ...
  Future<void> getApi(String period) async {
    ...
    try{
      http.Response response = await http.post(
          Uri.parse('${baseUrl}cleanly/v1/deposit'),
          body: {
            'key': apiKey,
            'period': period,
          }
